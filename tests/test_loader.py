...
class TestExtensionLoader(TestCase):
    ...
    def test_init_no_extensions(self):
        core_config = {'plugin_dir': [os.path.join(os.path.curdir, 'tests', 'no-extensions')]}
    ...
    def test_init_with_extensions(self):
        core_config = {'plugin_dir': [os.path.join(os.path.curdir, 'tests', 'extensions')]}
        loader = ExtensionLoader(core_config)
        result = loader.ext_init()
    ...
    def test_ext_load_new(self):
        core_config = {'plugin_dir': [os.path.join(os.path.curdir, 'tests', 'zip-extensions')]}
        loader = ExtensionLoader(core_config)
